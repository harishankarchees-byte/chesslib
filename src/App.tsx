...
function Routes() {
  ...
  if (path === '/add') return <AddBookPage />;
  ...
  if (bookMatch) return <BookDetailPage bookId={bookMatch[1]} />;

  // /copy/:code
  const copyMatch = path.match(/^\/copy\/(.+)$/);
  if (copyMatch) return <CopyDetailPage code={copyMatch[1]} />;

  // /edit/:id
  const editMatch = path.match(/^\/edit\/(.+)$/);
  if (editMatch) return <EditBookLoader bookId={editMatch[1]} />;
